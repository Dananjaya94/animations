...
export default function Singularity() {
    ...
    useEffect(() => {
        ...
        const transition = () => {
            ...
            const valEls = Array.from(container.querySelectorAll('.sg-val'));
            const textEls = [mainTitleRef.current, statusTextRef.current, ...valEls];
            gsap.to(textEls, {
                opacity: 0, duration: 0.8, onComplete: () => {
                    if (mainTitleRef.current) mainTitleRef.current.innerText = s.title;
                    if (statusTextRef.current) {
                        statusTextRef.current.innerText = s.status;
                        statusTextRef.current.style.color = s.color;
                        statusTextRef.current.style.borderColor = s.color;
                    }
                    if (velValRef.current) {
                        velValRef.current.innerText = s.vel;
                        velValRef.current.style.color = s.color;
                    }
                    gsap.to(textEls, { opacity: 1, duration: 1.2 });
                }
            });
        };

        triggerTransitionRef.current = transition;
        ...
        return () => {
            ...
            gsap.killTweensOf([diskMaterial.uniforms.uMorph, diskMaterial.uniforms.uCompression, diskMaterial.uniforms.uIntensity, diskMaterial.uniforms.uOrbitScale, auraMat.uniforms.uIntensity, controls, camera.position, camControl]);
    ...
    return (
        <div className="sg-container" ref={containerRef}>
            <canvas className="sg-canvas" ref={canvasRef} />
            <div className="sg-vignette" />
            <div className="sg-overlay">
                <div className="sg-header">
                    <div className="sg-title" ref={mainTitleRef}>Stable Singularity</div>
                    <div className="sg-status-pill" ref={statusTextRef}>Topology: Nominal</div>
                    <button
                        type="button"
                        className="sg-topology-btn"
                        onClick={() => triggerTransitionRef.current?.()}
                    >
                        Topology
                    </button>
                </div>
                <div className="sg-hud-bottom">
                    <div>
                        <div className="sg-metric">MASS_INDEX: <span className="sg-val">4.2M SOL</span></div>
                        <div className="sg-metric">LENSING: <span className="sg-val">SCHWARZSCHILD</span></div>
                    </div>
                    <div style={{ textAlign: 'right' }}>
                        <div className="sg-metric">RELATIVITY: <span className="sg-val" ref={velValRef}>0.45c</span></div>
                        <div className="sg-metric">RADIATION: <span className="sg-val">DETECTION ON</span></div>
                    </div>
                </div>
            </div>
        </div>
